...
def ephem_date_to_datetime(ephem_date):
    ...
    return datetime(int(date_[0]), int(date_[1]), int(date_[2]), int(time_[0]), int(time_[1]), int(time_[2]))
...
class Ephemerids(object):
    ...
    def calculate_moon_ephems(self, thedate):
        ...
        # Moon rise and set
        self.moon_prev_rise = \
            ephem_date_to_datetime(self.Observatory.previous_rising(ephem.Moon()))
        self.moon_prev_set = \
            ephem_date_to_datetime(self.Observatory.previous_setting(ephem.Moon()))
        self.moon_next_rise = \
            ephem_date_to_datetime(self.Observatory.next_rising(ephem.Moon()))
        self.moon_next_set = \
            ephem_date_to_datetime(self.Observatory.next_setting(ephem.Moon()))
...
class SQMData(object):
    ...
    def process_rawdata(self, Ephem):
        ...
        for k, line in enumerate(self.raw_data):
            # DateTime extraction
            utcdatetime = process_datetimes(line[0])
            localdatetime = process_datetimes(line[1])
...
class Plot(object):
    def __init__(self, Data, ephem):
        ...
        self.plot_moon_phase(ephem)
        self.plot_twilight(ephem)

    def plot_moon_phase(self, ephem):
        """
        ...
        if ephem.moon_next_rise > ephem.moon_next_set:
            # We need to divide the plotting in two phases (pre-midnight and after-midnight)
            self.thegraph_time.axvspan(ephem.moon_prev_rise + timedelta(hours=config._local_timezone),
            ...
            self.thegraph_time.axvspan(ephem.moon_next_rise + timedelta(hours=config._local_timezone),
                                       ephem.moon_next_set + timedelta(hours=config._local_timezone),
                                       edgecolor='#d62728', facecolor='#d62728', alpha=0.1, clip_on=True)
    ...
    def make_subplot_sunalt(self, twinplot=0):
        ...
        self.thegraph_sunalt.set_title( \
            'Sky Brightness (' + config.device_shorttype + '-' + \
    ...
    def plot_data_sunalt(self, Data, Ephem):
        ...
        self.thegraph_sunalt.text(0.00, 1.015,
                                  config.device_shorttype + '-' + config.observatory_name + ' ' * 5 + 'Serial #' + str(
                                      Data.serial_number), color='0.25', fontsize='small', fontname='monospace',
...
def save_stats_to_file(Night, NSBData, Ephem):
    ...
    Header = \
        '# Summary statistics for ' + str(config.device_shorttype + '_' + config.observatory_name) + '\n' + \
    ...
    statistics_filename = \
        ...
        str(config.device_shorttype + '_' + config.observatory_name) + '.dat'
...
def make_plot(input_filename=None, send_emails=False, write_stats=False):
    ...
    if input_filename is None:
        input_filename = config.current_data_directory + \
                         '/' + config.device_shorttype + '_' + config.observatory_name + '.dat'
    ...
    output_filenames = [ \
        str("%s/%s_%s.png" % (config.current_data_directory, \
                              config.device_shorttype, config.observatory_name)), \
...
if __name__ == '__main__':
    ...
    InputArguments = settings.ArgParser(input_file=True)
