...
config._device_type = config.device_type.replace('_', '-')
...
class Device(Observatory):
    def standard_file_header(self):
        ...
        header_content = header_content.replace('$DATA_SUPPLIER', str(config.data_supplier))
        header_content = header_content.replace('$LOCATION_NAME', str(config.device_locationname))
        header_content = header_content.replace('$OBSLAT', str(config.observatory_latitude))
        header_content = header_content.replace('$OBSLON', str(config.observatory_longitude))
        header_content = header_content.replace('$OBSALT', str(config.observatory_altitude))
        header_content = header_content.replace('$OFFSET', str(config.offset_calibration))
        ...
        if config.local_timezone == 0:
            ...
            header_content = header_content.replace('$TIMEZONE', 'UTC' + str(config.local_timezone))

        header_content = header_content.replace('$PROTOCOL_NUMBER', str(self.protocol_number))
        header_content = header_content.replace('$MODEL_NUMBER', str(self.model_number))
        header_content = header_content.replace('$FEATURE_NUMBER', str(self.feature_number))
        header_content = header_content.replace('$SERIAL_NUMBER', str(self.serial_number))

        header_content = header_content.replace('$IXREADOUT', remove_linebreaks(self.ix_readout))
        header_content = header_content.replace('$RXREADOUT', remove_linebreaks(self.rx_readout))
        header_content = header_content.replace('$CXREADOUT', remove_linebreaks(self.cx_readout))
    ...
    def define_filenames(self):
        ...
        date_time_file = self.local_datetime(self.read_datetime()) - datetime.timedelta(hours=12)
        date_file = date_time_file.date()
        yearmonth = str(date_file)[0:7]
        yearmonthday = str(date_file)[0:10]

        self.monthly_datafile = config.monthly_data_directory + "/" + config.device_shorttype + "_" + config.observatory_name + "_" + yearmonth + ".dat"
        self.daily_datafile = config.daily_data_directory + "/" + yearmonthday.replace('-',
                                                                                       '') + '_120000_' + config.device_shorttype + '-' + config.observatory_name + '.dat'
        self.current_datafile = config.current_data_directory + "/" + config.device_shorttype + "_" + config.observatory_name + ".dat"
...
class SQMLU(SQM):
    ...
    def read_metadata(self, tries=1):
        ...
        try:
            # Sanity check
            assert len(msg) == _meta_len_ or _meta_len_ is None
            assert "i," in msg
            self.metadata_process(msg)
        except:
            tries -= 1
    ...
    def read_data(self, tries=1):
        """
        Read the SQM and format the Temperature, Frequency and NSB measures
        :param tries: Number of tries to read for.
        :return:
        """
        self.s.write('rx')
        time.sleep(1)
        ...
        try:
            # Sanity check
            assert (len(msg) == _data_len_ or _data_len_ is None)
            assert ("r," in msg)
            self.data_process(msg)
        except:
            tries -= 1
